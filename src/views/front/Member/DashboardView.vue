...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div
        class="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-all duration-200 transform hover:-translate-y-1 cursor-pointer border border-gray-100"
        @click="router.push('/member/balance')"
      >
        <div class="flex items-center justify-between mb-4">
          <div class="bg-teal-100 p-3 rounded-lg">
            <i class="fas fa-wallet text-2xl text-teal-600" />
          </div>
          <span class="text-sm font-medium text-gray-500">帳戶餘額</span>
        </div>
        <h3 class="text-2xl font-bold text-gray-900 mb-2">
          ${{ (userInfo?.balance ?? 0).toLocaleString() }}
        </h3>
        <p class="text-sm text-gray-600">
          剩餘額度
        </p>
        <div class="mt-4 flex items-center text-teal-600 hover:text-teal-700">
          <span class="text-sm font-medium">儲值 / 明細</span>
          <i class="fas fa-arrow-right ml-2 text-xs" />
        </div>
      </div>
    </div>
